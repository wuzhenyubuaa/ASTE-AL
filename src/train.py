...
def train_full(Dataset, Network):
    ## dataset
    cfg    = Dataset.Config(datapath=args.dir_dataset, savepath=args.dir_checkpoints, mode=args.mode, batch=args.batchsize,
                            lr=args.lr, momen=args.moment, decay=args.decay, epoch=args.epoch, output_channel= args.output_channel)
...
def train_point(Dataset, Network):
    ## dataset
    cfg    = Dataset.Config(datapath=args.dir_dataset, savepath=args.dir_checkpoints, mode='train', batch=args.batchsize,
                            lr=args.lr, momen=args.moment, decay=args.decay, epoch=args.epoch, output_channel= args.output_channel)
    data   = Dataset.Data(cfg, args)
    ...
    for nth_query in range(n_stages):
        ...
        for epoch in range(cfg.epoch):
            ...
            for step, (image, mask, queries, name) in enumerate(dataloader):
                ...
                sw.add_scalars('loss', {'loss1u':loss1u.item(), 'loss2u':loss2u.item(), 'loss2r':loss2r.item(), 'loss3r':loss3r.item(), 'loss4r':loss4r.item(), 'loss5r':loss5r.item()}, global_step=global_step)
                if step%10 == 0:
                    print('%s | nth_query:%d | step:%d/%d/%d | lr=%.6f | loss=%.6f'%(datetime.datetime.now(), nth_query ,global_step, epoch+1, cfg.epoch, optimizer.param_groups[0]['lr'], loss.item()))

                gc.collect()

            # save the intermedia training data result, r
            print(name[0])
