...
class ResNet(nn.Module):
    ...
    def initialize(self):
        self.load_state_dict(torch.load('/media/jack/新加卷/mywork/pre-trained-model/resnet50-19c8e357.pth'), strict=False)
...
class F3Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.linearp1 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)
        self.linearp2 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)

        self.linearr2 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)
        self.linearr3 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)
        self.linearr4 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)
        self.linearr5 = nn.Conv2d(64, self.cfg.output_channel, kernel_size=3, stride=1, padding=1)
        self.initialize()
