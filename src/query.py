...
class QuerySelector:
    ...
    def _select_queries(self, uc_map, name):
        ...
        ind_queries = uc_map.topk(k=k,
                                  dim=0,
                                  largest=self.args.query_strategy in ["entropy", "least_confidence"]).indices.cpu().numpy()
    ...
    def __call__(self, nth_query, model):
        ...
        # with torch.no_grad():
        #     for batch_ind, (image, gt, queries, name, image_gray, image_hflip ) in tqdm(enumerate(self.dataloader_query)):
        #         image, gt = image.permute(0,3,1,2).cuda().float(), gt.cuda().float()
        #         mask = queries.squeeze(dim=0)
        #
        #         list_names.append(name[0])
        ...
        #         prob = F.softmax(out2u, dim=1)
        #
        #         # the image_gray and flip image
        #         image_gray = image_gray.permute(0, 3, 1, 2).cuda().float()
        #         _, out2u_gray, _, _, _, _ = model(image_gray)
        #         prob_gray = F.softmax(out2u_gray, dim=1)
        #
        #         image_hflip = image_hflip.permute(0, 3, 1, 2).cuda().float()
        #         _, out2u_flip, _, _, _, _ = model(image_hflip)
        #         prob_flip = F.softmax(out2u_flip, dim=1)
        #
        #         prob_sum = (prob + prob_gray + prob_flip)/3
        #
        #         uc_map = self.uncertainty_sampler(prob_sum).squeeze(dim=0)  #
        #
        #         # uc_map = self.uncertainty_sampler(prob).squeeze(dim=0) # only the original image
        #
        #         # exclude pixels that are already annotated, belong to the void category
        #         uc_map[mask] = 0.0 if self.args.query_strategy in ["entropy", "least_confidence"] else 1.0
        #
